...
def solve(edges: list[tuple[str, str]]) -> list[str]:
    ...
    def neighbour_sort_key(n: str) -> tuple[int, str]:
        return 1 if is_gateway(n) else 0, n

    def bfs_with_parents(start: str) -> tuple[dict[str, int], dict[str, str]]:
        dist = {start: 0}
        parent = {}
        q = deque([start])
        while q:
            ...
            for neighbour in sorted(graph[cur], key=neighbour_sort_key):
    ...
    def find_nearest_gate(virus):
        dist, parent = bfs_with_parents(virus)
    ...
    def rebuild_predposledniy_make_first_step(target: str, parent: dict[str, str], start: str) -> tuple[str, str]:
    ...
    while True:
        ...
        if off_edge_node not in graph.get(off_edge_gw, set()) or off_edge_gw not in graph.get(off_edge_node, set()):
